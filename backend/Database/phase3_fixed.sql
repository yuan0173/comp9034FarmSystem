-- ============================================
-- Phase 3: SQLite 数据库迁移脚本 - 高级安全特性 (修复版)
...
DROP TABLE IF EXISTS BiometricVerifications;

-- ============================================
-- 2. BiometricVerification 表创建 (Tan 设计核心)
-- ============================================
...
-- ============================================
-- 4. 为生物识别数据表添加测试数据 (如果还没有)
-- ============================================

-- 检查是否已有生物识别数据，如没有则添加
INSERT OR IGNORE INTO BiometricData (StaffId, BiometricType, TemplateData, Salt, TemplateHash, DeviceEnrollment, IsActive, CreatedAt, UpdatedAt)
SELECT 
    Id as StaffId,
    'fingerprint' as BiometricType,
    'TEMPLATE_' || Id || '_FINGERPRINT_DATA_' || abs(random()) as TemplateData,
    lower(hex(randomblob(16))) as Salt,
    lower(hex(randomblob(32))) as TemplateHash,
...
FROM Staffs 
WHERE IsActive = 1 
AND Id NOT IN (SELECT DISTINCT StaffId FROM BiometricData WHERE StaffId IS NOT NULL);
...
INSERT INTO BiometricVerifications (
    ...
    ConfidenceScore, ProcessingTime, IpAddress, UserAgent
)
SELECT 
    s.Id,
    bd.Id,
    ...
    ROUND((850 + abs(random() % 150)) / 1000.0, 3) as ConfidenceScore,  -- 0.850-0.999
    500 + abs(random() % 1000) as ProcessingTime,  -- 500-1500ms
...
VALUES 
    (2, 'no_match', 0.456, 'No matching template found', 950, '192.168.1.101', 'BiometricDevice/1.0'),
    (2, 'no_match', 0.321, 'No matching template found', 1200, '192.168.1.101', 'BiometricDevice/1.0'),
    (1, 'poor_quality', 0.234, 'Poor fingerprint quality', 300, '192.168.1.100', 'BiometricDevice/1.0');
...
SELECT 
    ...
    CASE 
        ...
        ELSE 'CLOCK_OUT'
    END as EventType,
    datetime('now', '-' || (bv.Id * 10) || ' minutes') as Timestamp,  -- 错开时间
    'Biometric verification ID: ' || bv.Id as Notes,
    datetime('now', '-' || (bv.Id * 10) || ' minutes') as CreatedAt
...
SELECT 
    ...
    DeviceId,
    COUNT(*) AS EventCount,
    'Multiple consecutive verification failures' AS Description,
...
SELECT 
    ...
    DeviceId,
    COUNT(*) AS EventCount,
    'Successful verifications with suspiciously low confidence (<0.800)' AS Description,
    MAX(CreatedAt) AS LastOccurrence
FROM BiometricVerifications
WHERE CreatedAt >= datetime('now', '-24 hours')
