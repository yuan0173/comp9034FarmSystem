...
## 交互顺序（强制：先解释、后授权、再执行）
- 1) 接收错误与上下文：贴关键日志/报错片段（仅必要行），标注环境与触发场景。
- 2) 通俗解释（必须）：用大白话说清“现象 → 根因 → 影响/风险”。
- 3) 解决方案对比（必须）：至少 A/B 两案，说明改动范围/影响/回滚，推荐其一并给出理由。
- 4) 授权请求（必须）：列出“将执行的具体操作清单”（文件/接口/命令/分支/PR），等待授权后再动。
- 5) 分步执行（逐步授权）：每步执行前一句话说明预期；得到授权后执行；完成后回报结果。
- 6) 推送与 PR（英文-only）：提交与 PR 正文全英文，禁止中文；不夹带无关改动。
- 7) 合并后验证（必须）：跑健康检查/冒烟测试；若失败，给出“最小回滚或补丁”并请求授权。
- 8) 通俗总结与下一步：用大白话收尾“已实现/未实现/下一步与授权点”。

## 会话初始化清单（每次启动必须勾选）
- [ ] 我已阅读本文件的“沟通与解释风格（强制）”与“交互顺序（强制）”，并将严格执行。
- [ ] 我将先解释、再请求授权、再执行修改；仓库内容与 PR/Commit 全英文，Chat 用中文通俗解释。

## 合并与清理（强制自动化）
- Merge（由维护者执行）
  - `gh pr merge <PR#> --merge --delete-branch`（合并并删除远端分支）
- Sync main
  - `git checkout main && git pull --ff-only`
- Cleanup (Local)
  - `git branch -d <merged-branch>`（仅删除已合并的当前工作分支）
  - 可选批量清理：`git branch --merged | grep -v "main" | xargs -n 1 git branch -d`
- Cleanup (Remote refs)
  - `git fetch -p`（prune 已删除的远端分支引用）

Note
- PR 与 Commit 一律英文；Chat 用中文通俗解释。
- 仅对已合并分支进行清理；未合并分支严禁删除。
